...
public class User extends Thread implements ServerInterFace {
	...
	private void netWork() {
		try {
			...
			serverGUI.getTextArea().append("<<" + userName + ">> 입장\n");

//			// 기존 유저에게 알림
			netWork.announCement("NewUser/" + userName);
			...
			public void run() {
				while (true) {
					try {
						...
						serverGUI.getTextArea().append("<<" + userName + ">>" + msg + "\n");
						netWork.inmessage(msg);
					} catch (Exception e) {
						try {
							// 나갔음
							serverGUI.getTextArea().append(userName + ": 사용자 접속이 끊어졌습니다.\n");
							dataOutputStream.close();
							dataInputStream.close();
							userSocket.close();
							vc.remove(this);
							chRoom.remove(this);
							netWork.announCement("UserOut/" + userName);
